// BentoGrid — three-tile section showing rod specs, replacement guarantee, and inshore rod.
import React from 'react';
import { motion } from 'framer-motion';
import { Ruler, ShieldAlert, ArrowUpRight } from 'lucide-react';
import inshoreImg from '../../assets/gallery-2.jpg';
...
const BentoGrid: React.FC<BentoGridProps> = ({ product }) => {
  const specs = product?.detailedSpecs;
  const specRows = [
    { label: 'Length',          value: specs?.length     ?? '9\'2"' },
    { label: 'Pieces',          value: specs?.pieces     ?? '5-Piece Travel' },
    { label: 'Action',          value: specs?.action     ?? 'Mod-Fast' },
    { label: 'Casting Weight',  value: specs?.lureWeight ?? '1-3 oz' },
    { label: 'Recommended Line',value: specs?.lineRating ?? '15-20 lb Braid' },
    { label: 'Guides',          value: 'Titanium / SiC' },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-3">
        ...
        >
          <div className="flex justify-between items-start">
            ...
            <span className="font-mono text-xs border border-black px-2 py-1">SPEC_SHEET_v2</span>
          </div>
          
          <div className="space-y-6">
            <div className="space-y-2">
              {specRows.map(({ label, value }) => (
                <div key={label} className="flex justify-between border-b border-neutral-300 pb-1">
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <span className="font-mono text-xs border border-white/30 px-2 py-1 uppercase">Guarantee</span>
          </div>
