...
const Reviews: React.FC = () => {
  return (
    <section className="w-full border-b border-black bg-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border-b border-black">
        <div className="p-8 md:p-12 border-b md:border-b-0 border-black md:border-r flex items-center justify-between bg-neutral-50">
           ...
           <div className="text-right">
             <span className="text-5xl font-black tracking-tighter block" aria-label="4.9 out of 5 stars">4.9</span>
             <div className="flex gap-1 justify-end mt-1" aria-hidden="true">
               {[1, 2, 3, 4, 5].map((i) => (
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex gap-1" aria-label={`${review.rating} out of 5 stars`}>
                  {[...Array(review.rating)].map((_, i) => (
                    <Star key={i} className="w-3 h-3 fill-black text-black" aria-hidden="true" />
                  ))}
                </div>
      ...
      <button
        type="button"
        className="w-full border-t border-black p-4 flex justify-center items-center hover:bg-black hover:text-white transition-colors group"
        aria-label="Read all 51 customer reviews"
      >
