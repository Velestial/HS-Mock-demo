                ...
                >
                    <AnimatePresence mode="popLayout">
                        {sortedProducts.map((item, index) => {
                            const isWide = item.categoryId === 'rod' || item.categoryId === 'bundle';
                            const nodes = [];

                            // Editorial break tile after every 8th product
                            if (index === 8) {
                                nodes.push(
                                    <div key="editorial-break" className="lg:col-span-4 bg-black text-white p-8 md:p-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
                                        <p className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none">
                                            Any Coast.<br />Any Condition.
                                        </p>
                                        <p className="font-mono text-xs text-neutral-400 uppercase leading-relaxed max-w-sm">
                                            Florida-designed for the traveling angler. Gulf flats to open surf — built for wherever the tide takes you.
                                        </p>
                                    </div>
                                );
                            }
                            ...
                            nodes.push(
                                <motion.div
                                    ...
                                    className={`bg-white group hover:bg-neutral-50 transition-colors relative ${isWide ? 'lg:col-span-2 flex flex-row' : 'p-8 flex flex-col h-full'}`}
                                >
                                    {isWide ? (
                                        <>
                                            {/* Wide card: horizontal layout */}
                                            <div
                                                className="w-1/2 relative flex items-center justify-center p-8 cursor-pointer overflow-hidden"
                                                onClick={() => onProductSelect && onProductSelect(item)}
                                            >
                                                <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
                                                    <span className="font-mono text-xs text-neutral-400 uppercase border border-neutral-200 px-2 py-1 bg-white">
                                                        {item.categoryId || 'GEAR'}
                                                    </span>
                                                    {item.tag && (
                                                        <span className="font-mono text-xs text-white uppercase border border-black px-2 py-1 bg-black">
                                                            {item.tag.replace('_', ' ')}
                                                        </span>
                                                    )}
                                                </div>
                                                <img
                                                    src={item.image}
                                                    alt={item.name}
                                                    className="w-full h-48 object-contain mix-blend-multiply group-hover:scale-105 transition-transform duration-500"
                                                />
                                            </div>
                                            <div className="w-1/2 p-8 flex flex-col justify-between border-l border-black/10">
                                                <div>
                                                    <h3
                                                        className="text-2xl font-black uppercase tracking-tight leading-none mb-3 cursor-pointer hover:text-neutral-600 transition-colors"
                                                        onClick={() => onProductSelect && onProductSelect(item)}
                                                    >
                                                        {item.name}
                                                    </h3>
                                                    <span className="block font-mono text-xs text-neutral-500 uppercase mb-4">{item.specs}</span>
                                                    <p className="text-sm text-neutral-600 font-medium uppercase leading-relaxed">
                                                        {item.description}
                                                    </p>
                                                </div>
                                                <div className="mt-6 flex items-center justify-between">
                                                    <span className="font-mono text-xl font-bold">${item.price}</span>
                                                    <button
                                                        onClick={() => handleAddToCart(item)}
                                                        className="bg-white border border-black text-black px-6 h-10 flex items-center gap-2 hover:bg-black hover:text-white transition-all uppercase font-bold text-xs tracking-widest"
                                                    >
                                        ...
                                        <>
                                            ...
                                            <div className="flex-grow flex flex-col justify-between">
                                                ...
                                                <button
                                                    onClick={() => handleAddToCart(item)}
                                                    className="w-full bg-white border border-black text-black h-10 flex items-center justify-center gap-2 hover:bg-black hover:text-white transition-all uppercase font-bold text-xs tracking-widest"
                                                >
                                                    <Plus className="w-3 h-3" />
